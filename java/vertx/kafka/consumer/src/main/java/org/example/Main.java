...
import org.example.verticle.HealthVerticle;
import org.example.verticle.KafkaConsumerVerticle;
	...
	public static void main(String[] args) {
		...
		getConfig(vertx)
			.flatMap(conf -> {
				...
				return run(Flowable.just(HealthVerticle.class, KafkaConsumerVerticle.class), deploymentOptions, vertx);
			})
			.subscribe(
				resList ->
					resList.forEach(res ->
